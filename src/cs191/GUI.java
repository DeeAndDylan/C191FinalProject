...
public class GUI extends JFrame implements MidiListener { // GUI is-a JFrame
	...
	private static JComboBox<InstrumentType> instrumentSelector;
	...
	public GUI(SampleHandler sample) {
		...
		setContentPane(background); // Set as content pane
		...
		// Creates the PianoContainer to hold PianoButtons (with lowered position)
		JPanel pianoContainer = new JPanel(new GridBagLayout());
		pianoContainer.setOpaque(false);
		GridBagConstraints gbc = new GridBagConstraints();
		gbc.gridx = 0;
		gbc.gridy = 1; // Positions it lower
		gbc.insets = new Insets(50, 0, 0, 0); // Adds 50px top margin
		gbc.anchor = GridBagConstraints.CENTER;
		pianoContainer.add(buttonPanel, gbc);
		background.add(pianoContainer, BorderLayout.CENTER);

		// Create and add the top panel with note label and instrument selector
